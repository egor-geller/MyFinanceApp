...
export default function Register() {
  ...
  function validate(): string | null {
    ...
    if (!isValidEmail(email)) return t('register.invalidEmail');
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (serverMsg === 'Email already registered') {
        ...
        setError(serverMsg ?? t('register.failed'));
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-white to-orange-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800 px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 p-8 space-y-5">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{t('register.title')}</h1>
          ...
          <p className="text-sm text-center text-slate-500">
            ...
            <Link to="/login" className="text-amber-500 hover:text-amber-600 font-medium hover:underline">
              {t('register.signIn')}
            </Link>
          </p>
        </form>
    ...
    <div className="min-h-screen flex items-center justify-center">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8 w-full max-w-sm space-y-4">
        ...
        <p className="text-sm text-center text-slate-500">
          {t('register.haveAccount')}{' '}
          <Link to="/login" className="text-amber-500 hover:underline">{t('register.signIn')}</Link>
        </p>
      </form>
    </div>
  );
